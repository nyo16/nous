...
{database_spy, spy_agent} =
  Testing.spy_tool("query_database",
...
called? = Testing.call_count(calc_agent) > 0
...
IO.puts(
  "Search called: #{Testing.call_count(search_agent) > 0} (#{Testing.call_count(search_agent)} times)"
)

IO.puts(
  "Weather called: #{Testing.call_count(weather_agent) > 0} (#{Testing.call_count(weather_agent)} times)"
)
...
defmodule MyAgentTest do
  ...
  test "agent uses search tool for queries" do
    ...
    # Assertions
    assert Testing.call_count(spy_agent) > 0
