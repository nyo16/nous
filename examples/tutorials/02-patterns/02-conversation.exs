...
# Nous AI - Conversation History Example
...
final_history = Enum.reduce(conversation_turns, message_history, fn user_message, history ->
  ...
  case Nous.run(agent, user_message, message_history: history) do
    {:ok, result} ->
...
IO.puts("   Nous.run(agent, message, message_history: history)")
IO.puts("")
IO.puts("✅ Use result.new_messages for updated history:")
IO.puts("   {:ok, result} = Nous.run(...)")
