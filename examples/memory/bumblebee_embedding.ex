# Local on-device embedding provider via Bumblebee + EXLA — a worked example
# of a `Nous.Memory.Embedding` provider that lives OUTSIDE Nous.
#
# This used to ship in `lib/` behind `if Code.ensure_loaded?(Bumblebee)`, which
# meant it never compiled or ran in Nous's own test suite (bumblebee drags
# Nx/EXLA, too heavy for the library's CI). The extension point is the
# behaviour, not the module: copy this file into your app, add
# `{:bumblebee, "~> 0.6"}` and `{:exla, "~> 0.9"}` to its deps, and add the two
# processes to your supervision tree:
#
#     children = [
#       {Registry, keys: :unique, name: MyApp.Memory.Embedding.Bumblebee.Registry},
#       MyApp.Memory.Embedding.Bumblebee.ServingSupervisor
#     ]
#
# then pass `embedding: MyApp.Memory.Embedding.Bumblebee` in `memory_config`.
# `examples/memory/local_bumblebee.exs` runs it end to end.

defmodule MyApp.Memory.Embedding.Bumblebee do
  @moduledoc """
  ...
  alias MyApp.Memory.Embedding.Bumblebee.{ServingHolder, ServingSupervisor}
  ...
  def get_or_start_holder(opts) do
    ...
    case Registry.lookup(MyApp.Memory.Embedding.Bumblebee.Registry, model_name) do
      [{pid, _}] ->
        {:ok, pid}
...
defmodule MyApp.Memory.Embedding.Bumblebee.ServingSupervisor do
  @moduledoc false
...
defmodule MyApp.Memory.Embedding.Bumblebee.ServingHolder do
  @moduledoc false
  use GenServer, restart: :transient

  require Logger

  @registry MyApp.Memory.Embedding.Bumblebee.Registry
