...
defmodule ResponseStructureEvaluator do
  ...
  def evaluate(actual, expected, _config) do
    ...
    checks =
      if expected[:min_sentences] do
        sentence_count = output |> String.split(~r/[.!?]+/, trim: true) |> length()
        passed = sentence_count >= expected.min_sentences

        [
          {:min_sentences, passed,
           "Sentence count: #{sentence_count} (min: #{expected.min_sentences})"}
          | checks
        ]
      else
        checks
    ...
    if checks == [] do
      ...
      %{
        ...
        details: %{
          checks:
            Enum.map(checks, fn {name, passed, msg} ->
...
case Eval.run(suite, timeout: 120_000) do
  {:ok, result} ->
    ...
    Enum.each(result.results, fn test_result ->
      # The runner folds the evaluator's :reason into :evaluation_details,
      # so pull it out and show the remaining details on their own.
      reason = test_result.evaluation_details[:reason] || test_result.error
      details = Map.drop(test_result.evaluation_details, [:reason])

      IO.puts("\n#{test_result.test_case_name}:")
      ...
      if details != %{} do
        ...
        case details do
          ...
          %{detected_sentiment: sentiment} ->
            ...
            IO.puts("    Positive words: #{details.positive_words}")
            IO.puts("    Negative words: #{details.negative_words}")
