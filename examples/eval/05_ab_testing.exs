...
case Eval.run_ab(suite,
  ...
  {:ok, comparison} ->
    ...
    if comparison.a.metrics_summary do
      IO.puts("  Latency p50: #{comparison.a.metrics_summary.p50_latency_ms}ms")
    end
    ...
    if comparison.b.metrics_summary do
      IO.puts("  Latency p50: #{comparison.b.metrics_summary.p50_latency_ms}ms")
    end
    ...
    # :score_diff is only present when both runs produced a metrics summary.
    score_diff = comparison.comparison[:score_diff]

    if score_diff do
      IO.puts("  Score difference: #{Float.round(score_diff, 3)}")
    end
