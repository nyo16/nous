...
# Yggdrasil AI - Multimodal Example (Vision + Text)
...
defmodule ImageAnalyzer do
    ...
    if File.exists?(image_path) do
      ...
      case File.read(image_path) do
        {:ok, image_data} ->
          ...
          case Yggdrasil.run(agent, multimodal_message) do
            {:ok, result} ->
              IO.puts("🤖 AI Analysis:")
  ...
  defp simulate_image_analysis(agent, image_path, prompt) do
    ...
    case Yggdrasil.run(agent, simulation_prompt) do
      {:ok, result} ->
...
vision_agent = Yggdrasil.new("lmstudio:qwen/qwen3-30b",
  instructions: """
