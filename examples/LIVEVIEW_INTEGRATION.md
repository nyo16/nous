# LiveView + Nous Integration Guide

## Overview

This guide shows how to integrate Nous AI agents with Phoenix LiveView, including proper process linking and cleanup.
...
defmodule MyAppWeb.StreamingChatLive do
  ...
  def handle_event("send_message", %{"text" => text}, socket) do
    ...
    agent_pid = spawn_link(fn ->
      ...
      case Nous.run_stream(agent, text) do
        {:ok, stream} ->
...
defmodule MyApp.PersistentAgentServer do
  ...
  def handle_call({:chat, message}, _from, state) do
    ...
    case Nous.run(state.agent, message, message_history: state.history) do
      {:ok, result} ->
        # Update history
...
### Related Examples
- ⚡ **[LiveView Streaming](liveview_streaming_example.ex)** - Real-time streaming chat (recommended)
- 📱 **[LiveView Chat](liveview_chat_example.ex)** - Complete chat interface with AgentServer
