...
Common issues and solutions for Yggdrasil AI development and deployment.
...
{:ok, stream} = Yggdrasil.run_stream(agent, long_prompt)

# 3. Implement timeout wrapper
def run_with_timeout(agent, prompt, timeout_ms \\ 60_000) do
  task = Task.async(fn -> Yggdrasil.run(agent, prompt) end)
...
def timed_run(agent, prompt) do
  ...
  result = case Yggdrasil.run(agent, prompt) do
...
def handle_long_query(agent, complex_query) do
  {:ok, stream} = Yggdrasil.run_stream(agent, complex_query)
...
defmodule ResponseCache do
  def cached_run(agent, prompt) do
    ...
    case get_cached_response(cache_key) do
      ...
      :miss ->
        response = Yggdrasil.run(agent, prompt)
...
```bash
# 1. Check logs
tail -f /var/log/myapp/error.log | grep -i "agent\|yggdrasil"
...
defmodule HealthCheck do
  def run_full_diagnostics do
    IO.puts("🔍 Yggdrasil Health Check")
    IO.puts("========================")
...
case Yggdrasil.run(agent, "Say hello") do
  {:ok, result} ->
...
tool_agent = Yggdrasil.new("lmstudio:qwen/qwen3-30b",
  tools: [&TestTool.simple_tool/2]
)

case Yggdrasil.run(tool_agent, "Use the simple tool with test data") do
  {:ok, result} ->
...
**Still stuck?** Check the [GitHub issues](https://github.com/nyo16/yggdrasil/issues) or create a new issue with your diagnostic information.