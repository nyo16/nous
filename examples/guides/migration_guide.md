...
Guide for upgrading Yggdrasil AI between versions and migrating configurations.

## Quick Migration Checklist

**Upgrading Yggdrasil?** Follow these steps:
...
{:ok, stream} = Yggdrasil.run_stream(agent, prompt)
stream
|> Stream.each(fn
...
# Update usage
{:ok, result} = Yggdrasil.run(agent, prompt, message_history: messages)
```
...
agent = YggdrasilAI.create_agent(%{
  model_name: "gpt-3.5-turbo",
  system_prompt: "You are helpful",
  tools: [MyTools.weather/1]
})

response = YggdrasilAI.chat(agent, "Hello")

# ✅ Modern 0.4.x format
agent = Yggdrasil.new("openai:gpt-3.5-turbo",
...
# ❌ Old error handling
try do
  response = YggdrasilAI.chat(agent, message)
...
defmodule MigrationTest do
  ...
  describe "migration compatibility" do
    test "basic agent functionality works" do
      ...
      {:ok, result} = Yggdrasil.run(agent, "Test message")
    ...
    test "tool calling still works" do
      ...
      {:ok, result} = Yggdrasil.run(agent, "Use the test tool with input 'hello'")
    ...
    test "conversation history is preserved" do
      ...
      {:ok, result1} = Yggdrasil.run(agent, "My name is Alice")
      {:ok, result2} = Yggdrasil.run(agent, "What's my name?",
...
defmodule MigrationValidator do
  ...
  defp test_basic_agent do
    agent = Yggdrasil.new("lmstudio:qwen/qwen3-30b")
    {:ok, _result} = Yggdrasil.run(agent, "Hello")
  end
  ...
  defp test_tools do
    ...
    {:ok, result} = Yggdrasil.run(agent, "Use the test tool")
    assert result.usage.tool_calls > 0
  end
...
config :yggdrasil, api_key: "..."

# ✅ Fix: New nested configuration
config :yggdrasil, :providers,
...
agent = Yggdrasil.new("model:name",  # New model format
