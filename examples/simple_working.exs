...
# Simple working example of Yggdrasil AI
#
...
IO.puts("Yggdrasil AI - Simple Example")
IO.puts("=" |> String.duplicate(70))
IO.puts("")
...
case Yggdrasil.run(agent, "What is 2+2?") do
  {:ok, result} ->
...
case Yggdrasil.run(agent, "What is the capital of France?") do
  {:ok, result} ->
    IO.puts("A: #{result.output}")
    IO.puts("Tokens used: #{result.usage.total_tokens}")
