...
{:ok, result1} = Nous.run(agent, "How many characters are in 'Hello World'?")
...
{:ok, result2} = Nous.run(agent, "Replace 'cat' with 'dog' in the text 'The cat sat on the cat mat'")
...
{:ok, result3} = Nous.run(agent, "Split 'apple,banana,orange,grape' by comma")
...
{:ok, result4} = Nous.run(agent, "How many times does 'the' appear in 'the quick brown fox jumps over the lazy dog'?")
...
{:ok, result5} = Nous.run(agent, "Convert 'hello world' to uppercase")
...
{:ok, result6} = Nous.run(agent, "Capitalize each word in 'hello world from elixir'")
...
{:ok, result7} = Nous.run(agent, "Extract characters from position 0 to 5 in 'Hello World'")
...
{:ok, result8} = Nous.run(agent, "Does 'Hello World' contain 'World'?")
...
{:ok, result9} = Nous.run(agent, "Reverse the text 'Hello World'")
...
{:ok, result10} = Nous.run(agent, "Extract all words from 'The quick brown fox, jumps over the lazy dog!'")
...
{:ok, result11} = Nous.run(agent, "Is 'racecar' a palindrome?")
...
{:ok, result12} = Nous.run(agent, "Extract all numbers from 'I have 3 apples, 5 oranges, and 2.5 kg of grapes'")
