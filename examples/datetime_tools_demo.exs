...
{:ok, result1} = Nous.run(agent, "What day is today? Is it a weekend?")
...
{:ok, result2} = Nous.run(agent, "What time is it now in 12-hour format?")
...
{:ok, result3} = Nous.run(agent, "What day will it be 30 days from now?")
...
{:ok, result4} = Nous.run(agent, "How many days are there between today and 2025-12-25?")
...
{:ok, result5} = Nous.run(agent, "What are the dates for this week (Monday to Sunday)?")
...
{:ok, result6} = Nous.run(agent, "Tell me about the current month - what month is it and how many days?")
...
{:ok, result7} = Nous.run(agent, "What time is it in New York (America/New_York timezone)?")
