...
# Example: Using Yggdrasil with tools (function calling)
#
...
IO.puts("Testing Yggdrasil AI with Tools")
...
case Yggdrasil.run(agent, "What is 25 + 17?") do
  {:ok, result} ->
    IO.puts("\n✅ Success!")
    IO.puts("\nResponse:")
...
case Yggdrasil.run(agent, "Calculate (5 + 3) * 4. Show your work!") do
  {:ok, result} ->
    IO.puts("\n✅ Success!")
    IO.puts("\nResponse:")
    IO.puts(result.output)
...
case Yggdrasil.run(agent, "What time is it right now?") do
  {:ok, result} ->
    IO.puts("\n✅ Success!")
    IO.puts("\nResponse:")
    IO.puts(result.output)
