...
Comprehensive guide for deploying Nous AI agents in production environments.
...
def handle_request(request) do
  agent = Nous.new(model, instructions: get_instructions())

  Nous.run(agent, request.prompt,
...
defmodule MyApp.AgentServer do
  ...
  def handle_call({:chat, message}, _from, state) do
    case Nous.run(state.agent, message,
...
defmodule MyApp.FallbackAgent do
  ...
  defp attempt_providers([provider | remaining], prompt, context) do
    ...
    case Nous.run(agent, prompt, context) do
      {:ok, result} ->
...
defmodule MyApp.Telemetry do
  ...
  def handle_event([:nous, :agent, :run, :stop], measurements, metadata, _config) do
    ...
    :telemetry_metrics.counter([{:nous, :requests}, {:status, metadata.status}])
    :telemetry_metrics.distribution([{:nous, :duration}], measurements.duration)
    :telemetry_metrics.distribution([{:nous, :tokens}], metadata.tokens_used)
  end

  def handle_event([:nous, :tool, :execute, :stop], measurements, metadata, _config) do
    :telemetry_metrics.counter([{:nous, :tool_calls}, {:tool, metadata.tool_name}])
    :telemetry_metrics.distribution([{:nous, :tool_duration}], measurements.duration)
  end
end
```
...
def create_agent do
  ...
  Nous.new("anthropic:claude-sonnet-4-5-20250929", api_key: api_key)
end

# Async processing
def handle_chat(message) do
  Task.Supervisor.start_child(MyApp.TaskSupervisor, fn ->
    result = Nous.run(agent, message)
...
1. **Review examples**: Study the runnable scripts in [examples/](../../examples/README.md)
