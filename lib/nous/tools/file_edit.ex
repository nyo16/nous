defmodule Nous.Tools.FileEdit do
  ...
  def execute(
    ...
    with {:ok, safe_path} <- Nous.Tools.PathGuard.validate(file_path, ctx),
         ...
         {:ok, new_content, replaced} <-
           plan_replacement(content, old_string, new_string, replace_all, safe_path) do
      write_edit(safe_path, new_content, replaced)
    else
  ...
  # Decide what the edit should write, or refuse it: an `old_string` that
  # matches nothing is a mistake, and one that matches several times is
  # ambiguous unless the caller opted into `replace_all`. Returns the new
  # content together with the number of replacements it represents.
  defp plan_replacement(content, old_string, new_string, replace_all, safe_path) do
  ...
  defp write_edit(safe_path, new_content, replaced) do
