defmodule Yggdrasil.AgentServer do
  @moduledoc """
  GenServer wrapper for Yggdrasil agents with PubSub integration.

  This server:
  - Wraps an Yggdrasil agent (standard or ReAct)
  ...
  @type state :: %{
          session_id: String.t(),
          agent: Yggdrasil.Agent.t() | Yggdrasil.ReActAgent.t(),
  ...
  defp do_agent_run(server_pid, state, message, current_history) do
    ...
    result = case state.agent_type do
      :react ->
        Yggdrasil.ReActAgent.run(state.agent, message,
      ...
      :standard ->
        Yggdrasil.Agent.run(state.agent, message,
          message_history: message_history,
          cancellation_check: fn -> check_cancelled(server_pid) end
        )
    end
    ...
    case result do
      ...
      {:error, %Yggdrasil.Errors.ExecutionCancelled{}} ->
